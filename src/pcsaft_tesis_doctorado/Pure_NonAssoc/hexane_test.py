# Script to test density_model.py with hexane parameters and Held results.
...
m.epskb = pyo.Param(initialize=236.769054)  # [K]
...
# CONSTRAINTS
# OBJECTIVE
m.obj = pyo.Objective(expr=(m.press - m.press_calc)**2, sense=pyo.minimize)
...
pyo.SolverFactory('ipopt').solve(m, tee=True, options=options)
...
print(pyo.value(m.press_calc))
print("\nHexane test")
print("eta_pyomo - eta_held:     ", pyo.value(m.eta)-held_eta)
print("eta_pyomo / eta_held - 1: ", (pyo.value(m.eta)/held_eta)-1)
