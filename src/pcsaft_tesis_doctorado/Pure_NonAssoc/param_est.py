...
filepath = "/Users/arturobastias/PycharmProjects/PyomoPCSAFT/Pure_NonAssoc/methane_data.xlsx"
df = read_excel(filepath)

KBOLTZMANN = 1.380649e-23  # [J/K] Boltzmann constant
NAVOGADRO = 6.02214076e23  # [mol-1] Avogadro's number
data = df
data_index = pyo.RangeSet(1, len(df))

# %% MODEL
def create_pcsaft_constants():
    """First creation of pcsaft: creates sets, constants and PCSAFT parameters"""
    ...
    # VARIABLES (can be fixed if not estimating PCSAFT parameters)
    m.mseg = pyo.Var(initialize=86.177 * .0354812325, within=pyo.PositiveReals)  # Hexane
    m.sigma = pyo.Var(initialize=3.79829291e-10, within=pyo.PositiveReals)  # [m], not [A]
    m.epskb = pyo.Var(initialize=236.769054, within=pyo.PositiveReals)
...
def create_pcsaft_TPmethod(b, i):
    """First extension of pcsaft: creates temperature-only-dependent expressions and pressure"""

    set03 = b.model().set03
    set06 = b.model().set06

    a_const = b.model().a_const
    b_const = b.model().b_const
    ...
    # data = b.model().data
    e = pyo.ConcreteModel(name="Experiment " + str(i))
    # VARIABLES
    e.tmprt = pyo.Var(name="Temperature (K)", within=pyo.PositiveReals)
    e.press = pyo.Var(name="Pressure (Pa)", within=pyo.PositiveReals)
    # Initial values and fix variables
    e.tmprt.fix(data["Temperature (K)"][i])
    e.press = data["Vapor Pressure (Pa)"][i]

    # EQUATIONS
    e.d = pyo.Expression(expr=sigma * (1 - 0.12 * pyo.exp(-3 * epskb / e.tmprt)))
    e.aux_xi = pyo.Expression(set03, rule=lambda m, n: mseg * PI / 6 * e.d ** n)
    # e.deta_drho = pyo.Expression(expr=e.aux_xi[set03.last()])
    e.deta_drho = pyo.Expression(expr=e.aux_xi[3])

    # Dispersion
    e.aa = pyo.Expression(set06,
                          rule=lambda m, i_: a_const[0, i_] + ((mseg - 1) / mseg) * a_const[1, i_] + (
                                  (mseg - 1) / mseg) * ((mseg - 2) / mseg) * a_const[2, i_])
    e.bb = pyo.Expression(set06,
                          rule=lambda m, i_: b_const[0, i_] + ((mseg - 1) / mseg) * b_const[1, i_] + (
                                  (mseg - 1) / mseg) * ((mseg - 2) / mseg) * b_const[2, i_])
    e.mes3 = pyo.Expression(expr=mseg * (sigma ** 3) * epskb / e.tmprt)
    e.m2e2s3 = pyo.Expression(expr=(mseg ** 2) * (sigma ** 3) * (epskb / e.tmprt) ** 2)

    # Ideal TODO: Only include a_ideal when calculating T derivatives.
    h = 6.626070150e-34  # J s
    me = 9.10938291e-31  # 1/Kg TODO: Would be better to use Mw/Na of substance.
    e.beta = pyo.Expression(expr=1. / (e.tmprt * KBOLTZMANN))
    e.broglie_vol = pyo.Expression(expr=h / pyo.sqrt(2 * PI * me * e.beta))
    return None


def create_pcsaft_phase(b, p_name):
...
#def create_pcsaft_regression(b, p_name):


model = create_pcsaft_constants()
model.data = data
model.experiment_set = pyo.Set
model.phases_names = pyo.Set(initialize=["Liquid", "Vapor"])
model.phases = pyo.Block(model.phases_names, rule=create_pcsaft_phase)

# CONSTRAINTS
# model.pressure_constraint = pyo.Constraint(expr=(model.press - model.press_calc)**2/1e10 == 0)
model.lnphi_constraint = pyo.Constraint(expr=(model.phases[model.phases_names.first()].lnphi - model.phases[model.phases_names.last()].lnphi) ** 2 == 0)
